 ...
 */
async function getUnbilledTimeEntries(request: NextRequest, context: any) {
  try {
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to fetch unbilled time entries' },
      { status: 500 }
    )
  }
}

export const GET = withAuth(getUnbilledTimeEntries, { requireOrg: true })
